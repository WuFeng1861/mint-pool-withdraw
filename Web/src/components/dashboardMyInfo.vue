<template>
  <div class="dashboardMyInfo">
    ...
    <div class="myInfoRight shadowNormal">
      <div :class="{
          startMint: startMint === 'Connect',
          doing: startMint !== 'Connect',
          pcOnly: true
        }" @click="useStartMint">
        <el-icon>
          <img :src="getSvgPath('Start')" v-if="startMint === 'Connect'"/>
...
const startMint = computed(() => {
  return isMining.value ? 'Remain ' + useMintTimeRef().value : 'Connect';
});
const startMintMobile = computed(() => {
  return isMining.value ? useMintTimeRef().value : 'Connect';
});
