<template>
  <div class="leftNav">
    ...
    <el-menu
        ...
        router
    >
      <el-menu-item :index="`/Home${item.path}`" v-for="(item, index) in routerItems"
                    :class="$route.path=== '/Home'+item.path?'is-active':''">
        <!-- 这里可以放置图标 -->
        <el-icon :style="{
          width: item.size + 'px',
      ...
      <el-menu-item @click="toEcologyWeb">
        <el-icon>
...
const toEcologyWeb = () => {
  window.open(config.ECOLOGY_URL, '_blank')
}
</script>
