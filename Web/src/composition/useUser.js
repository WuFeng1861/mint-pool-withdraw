  ...
  if (user.exp > Math.floor(Date.now() / 1000)) {
    ...
    if (path.includes('wallet')) {
      ...
      if(path.includes('wallet-swap')) {
        toRouteName ='wallet-swap';
        routerTo(toRouteName);
        return;
      }
      if(path.includes('wallet-mint')) {
        toRouteName ='wallet-mint';
        routerTo(toRouteName);
        return;
      }
      if(path.includes('exchange-faucet')) {
        toRouteName ='wallet-exchange-faucet';
        routerTo(toRouteName);
        return;
      }
