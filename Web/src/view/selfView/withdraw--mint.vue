      ...
      <el-table-column fixed="right" label="操作" width="100">
        <template #default="scope">
          <el-popconfirm
              ...
              icon-color="red"
              title="确定要取消同意该提币请求吗?"
              @confirm="executeTransaction(scope.$index, scope.row)"
          >
            <template #reference>
              <el-button v-if="scope.row.isFinish === '未完成'" link type="primary" size="small">执行提币</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
...
const contractAddress = config.MINT_CONTRACT_ADDRESS;
...
const connectWallet = async () => {
  ...
  etherWallet.updateTokenContract(config.MINT_CONTRACT_ADDRESS);
  getTotalList();
...
const withdrawHandleConfirm = (e) => {
  ...
  form.value.validate(async (valid) => {
    ...
    try {
      ...
      // 2.判断余额是否足够
      const balance = await etherWallet.getAddressEthBalance(contractAddress);
...
// 执行该交易
const executeTransaction = async (index, item) => {
  let i = item.index;
  if (!etherWallet.userAddress) {
    return elTip('请先连接钱包')
  }
  try {
    let res = await etherWallet.contractFn('executeTransaction', i);
    console.log(res, '执行该交易成功');
  }catch (e) {
    console.log(e, '执行该交易失败', e.message)
    if(e.message.indexOf('Transaction not confirmed') >= 0) {
      return elTip('您没有同意过该提币请求');
    }
  }
