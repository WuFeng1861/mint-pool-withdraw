...
import {
  ...
  addBillForMERC_Transfer, addBillForUSDT_Sys, addBillForUSDT_Transfer,
...
export async function transfer_ETH(fromId, amount, username, email, address) {
  ...
  try {
    ...
    // 判断转账人和接收人是否是同一个人
    if (fromId.toString() === userToId.toString()) {
      errorTip('不能转账给自己');
    }
    // 判断转账人的余额是否足够
    ...
    // 增加转账人的账单
    await userEthBillDao.create(fromId, SafeCalc.mul('-1', amount), tokenBillTypes.TRANSFER_OUT, null, createtime, t);
    // 判断接收人是否存在，增加接收人的余额
    if (!userToEth) {
      // 创建接收人用户ETH数据
    ...
    // 增加接收人的账单
    await userEthBillDao.create(userToId, amount, tokenBillTypes.TRANSFER_IN, null, createtime, t);
    // 提交事务
    await t.commit();
    return true;
  } catch (e) {
...
// 转账USDT
export async function transfer_USDT(fromId, amount, username, email, address) {
  // 判断转账金额 > 0
  if (SafeCalc.compare(amount, 0) !== '1') {
    throw new Error('转账金额必须大于0');
  }
  ...
  try {
    ...
    let userToUsdt = await userUsdtDao.findLock(userToId, t);
    // 获取转账人用户USDT数据并锁住
    let userFromUsdt = await userUsdtDao.findLock(fromId, t);
    // 判断转账人和接收人是否是同一个人
    if (userToId.toString() === fromId.toString()) {
      errorTip('不能转账给自己');
    }
    // 判断转账人的USDT余额是否足够
    if (SafeCalc.compare(userFromUsdt.amount, amount) === '-1') {
      errorTip('余额不足');
    }
    let createtime = getNow();
    // 增加钱包交易记录
    await addBillForUSDT_Transfer(fromId, userToId, createtime, amount, t);
    // 减少转账人的USDT余额
    let newBalanceFrom = SafeCalc.sub(userFromUsdt.amount, amount);
    await userUsdtDao.update(fromId, newBalanceFrom, t);
    // 增加转账人的账单
    await userUsdtBillDao.create(fromId, SafeCalc.mul(-1, amount), tokenBillTypes.TRANSFER_OUT, null, createtime, t);
    // 判断接收人是否存在，增加接收人的余额
    if (!userToUsdt) {
      // 创建接收人用户USDT数据
      await userUsdtDao.create(userToId, amount, t);
    } else {
      // 增加接收人的余额
      let newBalanceTo = SafeCalc.add(userToUsdt.amount, amount);
      await userUsdtDao.update(userToId, newBalanceTo, t);
    }
    // 增加接收人的账单
    await userUsdtBillDao.create(userToId, amount, tokenBillTypes.TRANSFER_IN, null, createtime, t);
    // 提交事务
    await t.commit();
    return true;
  } catch (e) {
...
// 获取USDT的交易记录
export async function getTransferRecords_USDT(id, startId, pageSize) {
  const coinName = 'USDT';
  let user = await userDao.getUserLock(id);
  if (!user.address) {
    throw new Error('请先绑定地址');
...
// 获取USDT的交易记录数
export async function getTransferRecordsCount_USDT(id) {
  const coinName = 'USDT';
  let user = await userDao.getUserLock(id);
  if (!user.address) {
    throw new Error('请先绑定地址');
