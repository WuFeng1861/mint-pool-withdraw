...
import {earningTypes, tokenBillTypes, userContributeTypes} from "../types/index.js";
...
// 获取用户usdt余额
export async function getUserUsdt(id) {
  let result = {usdtBalance: 0};
  let userUsdtData = await userUsdtDao.findLock(id);
  console.log(userUsdtData, id);
  if (!userUsdtData) {
    return result;
  }
  result.usdtBalance = userUsdtData.amount;
  return result;
}
...
export const getAddressFromCache = async (id) => {
...
export async function addBillForETH_Transfer(sender_id, receiver_id, createtime, amount, t) {
  ...
  const chain_hash = '';
  const type = tokenBillTypes.TRANSFER_IN_OUT;
...
// USDT添加账单
export async function addBillForUSDT_Sys(user_id, created_at, type, amount, chain_hash, t) {
  const coinName = "USDT";
  // sender""空字符串
  let sender = "";
  // 从user_id 获取address就是receiver
...
export async function addBillForUSDT_Transfer(sender_id, receiver_id, createtime, amount, t) {
  const coinName = "USDT";
  // 从sender_id 获取address就是sender
  let sender = await getAddressFromCache(sender_id);
  let receiver = await getAddressFromCache(receiver_id);
  ...
  const chain_hash = '';
  const type = tokenBillTypes.TRANSFER_IN_OUT;
  const hash = sha256(`swap-${createtime}-${sender}-${receiver}-${type}-${amount}-${chain_hash}`);
  const create_time = createtime;
  const fee = 0;
  const remark = '';
  // 添加记录
...
// Swap添加账单
export async function addBillForSwap_Sys(user_id, from_coin, to_coin, from_amount, to_amount, created_at, t) {
  const userAddress = await getAddressFromCache(user_id);
  if(!userAddress) {
    // 如果没有获取到user的地址，就不记录了
    // console.log(`user_id:${user_id} has no address`);
    return;
  }
  const hash = sha256(`swap-${created_at}-${userAddress}-${from_coin}-${to_coin}-${from_amount}-${to_amount}`);
  const create_time = created_at;
  const fee = 0;
  const remark = '';
  // 添加记录 from_coin 减少 from_amount
  if(from_coin === 'MERC') {
    await transferRecordDao.create(hash, from_coin, earningTypes.SWAP_OUT, userAddress, "", from_amount, create_time, fee, remark, t);
  } else {
    await transferRecordDao.create(hash, from_coin, tokenBillTypes.SWAP_OUT, userAddress, "", from_amount, create_time, fee, remark, t);
  }
  // to_coin 增加 to_amount
  const hashTo = sha256(`swap-${created_at}-${userAddress}-${from_coin}-${to_coin}-${from_amount}-${to_amount}-toCoin`);
  if(to_coin === 'MERC') {
    await transferRecordDao.create(hashTo, to_coin, earningTypes.SWAP_IN, "", userAddress, to_amount, create_time, fee, remark, t);
  } else {
    await transferRecordDao.create(hashTo, to_coin, tokenBillTypes.SWAP_IN, "", userAddress, to_amount, create_time, fee, remark, t);
  }
}
