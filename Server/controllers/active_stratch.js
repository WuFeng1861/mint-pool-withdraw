...
export const getStratchReward = async (req, res) => {
  ...
  try {
    ...
    // 添加钱包交易记录ETH
    await addBillForETH_Sys(id, createtime, tokenBillTypes.BLINDBOX_CONSUME, SafeCalc.mul(-1, config.OPEN_BOX_FEE), null, t);
    // 添加钱包交易记录MERC
    ...
    // 记录eth账单
    await userEthBillDao.create(user.id, -1 * config.OPEN_BOX_FEE, tokenBillTypes.BLINDBOX_CONSUME, null, createtime, t);
    // 增加余额
